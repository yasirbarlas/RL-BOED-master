...
from pyro.algos._functions import MetaRLAlgorithm
...
class PEARL(MetaRLAlgorithm):
    ...
    def _sample_data(self, indices):
        ...
        o = torch.as_tensor(o, device=device).float()
        a = torch.as_tensor(a, device=device).float()
        r = torch.as_tensor(r, device=device).float()
        no = torch.as_tensor(no, device=device).float()
        d = torch.as_tensor(d, device=device).float()
    ...
    def adapt_policy(self, exploration_policy, exploration_episodes):
        ...
        context = torch.as_tensor(ctxt, device=device).float()
        self._policy.infer_posterior(context)
