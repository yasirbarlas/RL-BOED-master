...
class UCBTrainer:
    ...
    def _start_worker(self):
        ...
        if self._plot:
            ...
            for i in self._algo._policies:
                self._plotter = Plotter()
                self._plotter.init_plot(self.get_env_copy(), i)
    ...
    def obtain_episodes(self,
        ...
        if agent_update is None:
            ...
            if policy is None:
                ...
                # failed otherwise.
                policies = self._algo._policies
                qfs = self._algo._qfs
            agent_update = [policy.get_param_values() for policy in policies]
            qf_update = [qf.state_dict() for qf in qfs]
            #print(len(agent_update))
        episodes = self._sampler.obtain_samples(
            itr, (batch_size or self._train_args.batch_size),
            agent_update=agent_update,
            env_update=env_update, qf_update=qf_update)
