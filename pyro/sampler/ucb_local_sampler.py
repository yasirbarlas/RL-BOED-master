...
from garage.sampler.ucb_worker_factory import UCBWorkerFactory


class UCBLocalSampler(Sampler):
    """Sampler that runs workers in the main process.
    ...
    def __init__(
        ...
        if isinstance(worker_factory, UCBWorkerFactory):
            self._factory = worker_factory
        else:
            self._factory = UCBWorkerFactory(
        ...
        self._qfs = self._factory.prepare_worker_messages(qfs)
        ...
        for worker, agent, env, qf in zip(self._workers, self._agents, self._envs, self._qfs):
    ...
    @classmethod
    def from_worker_factory(cls, worker_factory, agents, envs, qfs):
        ...
        """
        return cls(agents, envs, qfs, worker_factory=worker_factory)

    def _update_workers(self, agent_update, env_update, qf_update):
        ...
        qf_updates = self._factory.prepare_worker_messages(qf_update)
        env_updates = self._factory.prepare_worker_messages(env_update, preprocess=copy.deepcopy)
        #print(len(agent_updates), len(qf_updates), "j")
        for worker, agent_up, env_up, qf_up in zip(self._workers, agent_updates, env_updates, qf_updates):
            worker.update_agent(agent_up)
            worker.update_qf(qf_up)
            worker.update_env(env_up)

    def obtain_samples(self, itr, num_samples, agent_update, env_update=None, qf_update=None):
        ...
        """
        #print(len(agent_update), len(qf_update))
        self._update_workers(agent_update, env_update, qf_update)
        batches = []
        completed_samples = 0
        while True:
    ...
    def obtain_exact_episodes(self,
                              n_eps_per_worker,
                              agent_update,
                              env_update=None, qf_update=None):
